...
class SimpleWebServer(BaseHTTPRequestHandler):
    ...
    def PageReceipts(db):
        ...
        # Container für nebeneinander liegende Bereiche
        s+= '''
    ...
    def do_POST(self):
        ...
        if self.path == "/upload_belege":
            ...
            if 'multipart/form-data' not in content_type:
                self.respond(400, "Invalid content type")
                return
            ...
            if uploaded_files:
                message = f"{len(uploaded_files)} Datei(en) erfolgreich hochgeladen: {', '.join(uploaded_files)}"
            else:
